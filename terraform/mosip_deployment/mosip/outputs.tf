# MOSIP Services Outputs
# Note: data.kubernetes_config_map_v1.global is defined in main.tf
...
output "kafka_ui_url" {
  ...
  value       = var.enable_kafka ? "https://kafka.${data.kubernetes_config_map_v1.global.data["installation-domain"]}" : null
}
...
output "iam_status" {
  ...
  value = var.iam_enable ? {
    ...
    admin_url = "https://iam.${data.kubernetes_config_map_v1.global.data["installation-domain"]}/auth"
